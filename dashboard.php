...
// Matikan pesan error sementara agar desain tidak rusak jika database kosong
error_reporting(0);
...
// 1. Logika untuk memotong nama menjadi maksimal 2 kata (Misal: Muhammad Rasya Dzikri -> Muhammad Rasya)
$nama_panggilan = "Tamu";
...
// 2. Ambil foto profil dari database
...
// 3. Mengambil data dari tabel events dan ticket_categories
$events_terdekat = false;
$events_rekomendasi = false;
$events_slider = false;

if (isset($conn) && $conn) {
    // Ambil data untuk Slider (3 Event Terbaru dengan gambar cover)
    $query_slider = "
        SELECT id, judul, gambar_cover 
        FROM events 
        WHERE status = 'published' AND gambar_cover IS NOT NULL AND gambar_cover != ''
        ORDER BY created_at DESC 
        LIMIT 3
    ";
    $events_slider = mysqli_query($conn, $query_slider);

    // Ambil 4 event terdekat (Waktu event lebih besar/sama dengan waktu sekarang)
    $query_terdekat = "
        SELECT e.*, MIN(t.harga) as harga_termurah 
        FROM events e 
        LEFT JOIN ticket_categories t ON e.id = t.event_id 
        WHERE e.status = 'published' AND e.tanggal_waktu >= NOW()
        GROUP BY e.id 
        ORDER BY e.tanggal_waktu ASC 
        LIMIT 4
    ";
    $events_terdekat = mysqli_query($conn, $query_terdekat);

    // Ambil 4 event rekomendasi (Event terbaru yang dipublish, tidak peduli waktunya)
    $query_rekomendasi = "
        SELECT e.*, MIN(t.harga) as harga_termurah 
        FROM events e 
        LEFT JOIN ticket_categories t ON e.id = t.event_id 
        ...
        LIMIT 4
    ";
    $events_rekomendasi = mysqli_query($conn, $query_rekomendasi);
}
?>
<!DOCTYPE html>
<html lang="id">
<head>
  ...
  <title>TicketIn — Temukan Event Terbaik</title>
  ...
  <style>
    * { font-family: 'Poppins', sans-serif; }
    ...
    @keyframes badgePulse {
      0%, 100% { box-shadow: 0 0 0 0 rgba(245,196,0,0.5); }
...
<body class="bg-gray-50 font-poppins">
  ...
  <!-- ═══════════════ HERO SLIDER ═══════════════ -->
  <section id="beranda" class="pt-[96px] pb-6 max-w-7xl mx-auto px-6 relative overflow-hidden">
    <div class="relative w-full rounded-2xl overflow-hidden shadow-md group aspect-[16/9] md:aspect-[21/9] max-h-[400px]">
      
      <div id="slider" class="slider-track h-full">
        <?php if ($events_slider && mysqli_num_rows($events_slider) > 0): ?>
            <!-- LOOPING FOTO SLIDER DARI DATABASE -->
            <?php while($slide = mysqli_fetch_assoc($events_slider)): ?>
                <div class="slide h-full relative cursor-pointer" onclick="location.href='event-detail.php?id=<?= $slide['id'] ?>'">
                    <img src="<?= htmlspecialchars($slide['gambar_cover']) ?>" onerror="this.src='https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1400&q=80'" alt="<?= htmlspecialchars($slide['judul']) ?>" class="w-full h-full object-cover" />
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <!-- FALLBACK JIKA DATABASE KOSONG -->
            <div class="slide h-full relative cursor-pointer" onclick="location.href='event-detail.php'">
              <img src="https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1400&q=80" alt="Titik Temu Fest" class="w-full h-full object-cover" />
            </div>
            <div class="slide h-full relative cursor-pointer">
              <img src="https://images.unsplash.com/photo-1459749411175-04bf5292ceea?w=1400&q=80" alt="Rigem Fest" class="w-full h-full object-cover" />
            </div>
            <div class="slide h-full relative cursor-pointer">
              <img src="https://images.unsplash.com/photo-1533174072545-7a4b6ad7a6c3?w=1400&q=80" alt="Senang Senang Fest" class="w-full h-full object-cover" />
            </div>
        <?php endif; ?>
      </div>

      <button id="prev-btn" class="absolute left-4 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-gold text-white hover:text-navy-deep backdrop-blur-sm w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 z-10 opacity-0 group-hover:opacity-100">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
      </button>
      <button id="next-btn" class="absolute right-4 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-gold text-white hover:text-navy-deep backdrop-blur-sm w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 z-10 opacity-0 group-hover:opacity-100">
      ...
      <div id="dots" class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">
        <?php 
          // Memastikan titik (dots) menyesuaikan jumlah slide yang ditampilkan
          if ($events_slider && mysqli_num_rows($events_slider) > 0) {
              // Harus di reset cursor databasenya karena sebelumnya sudah dipakai di HTML di atas
              mysqli_data_seek($events_slider, 0); 
              $slide_count = mysqli_num_rows($events_slider);
          } else {
              $slide_count = 3;
          }
        ...
        ?>
            <button class="dot w-2 h-2 md:w-2.5 md:h-2.5 rounded-full <?= $i == 0 ? 'bg-white active' : 'bg-white/50' ?>" data-index="<?= $i ?>"></button>
        <?php endfor; ?>
      </div>
  ...
  <!-- ═══════════════ EVENT TERDEKAT ═══════════════ -->
  <section class="max-w-7xl mx-auto px-6 py-14 fade-in visible">
    <div class="flex items-center justify-between mb-8">
      ...
      <a href="events.php" class="text-navy-mid font-semibold hover:text-gold transition-colors text-sm flex items-center gap-1">
        Lihat semua
    ...
    <?php if ($events_terdekat && mysqli_num_rows($events_terdekat) > 0): ?>
        <!-- LOOPING DATA DINAMIS DARI DATABASE -->
        <?php while($row = mysqli_fetch_assoc($events_terdekat)): ?>
            <article class="event-card bg-white rounded-xl shadow-md overflow-hidden cursor-pointer group" onclick="location.href='event-detail.php?id=<?= $row['id'] ?>'">
                <div class="overflow-hidden h-44">
                <img src="<?= $row['gambar_cover'] ? htmlspecialchars($row['gambar_cover']) : 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=800&q=80' ?>" alt="<?= htmlspecialchars($row['judul']) ?>" class="w-full h-44 object-cover group-hover:scale-110 transition-transform duration-500" />
                </div>
                ...
                <h3 class="font-bold text-navy-deep text-base leading-snug"><?= htmlspecialchars($row['judul']) ?></h3>
                <p class="text-sm text-gray-400 mt-1 flex items-center gap-1">
                    ...
                    <?= date('d M Y, H:i', strtotime($row['tanggal_waktu'])) ?>
                </p>
                ...
                <div class="flex items-center justify-between mt-auto pt-3">
                    <p class="text-navy-mid font-bold text-sm">Rp<?= number_format($row['harga_termurah'] ?? 0, 0, ',', '.') ?></p>
                    <button class="bg-gold text-navy-deep text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-gold-light transition-all duration-200">Beli</button>
                </div>
                </div>
            </article>
        <?php endwhile; ?>
    <?php else: ?>
        <!-- PESAN KOSONG KETIKA BELUM ADA EVENT SAMA SEKALI -->
        <div class="col-span-1 md:col-span-2 lg:col-span-4 py-12 text-center bg-white rounded-2xl border border-gray-200 border-dashed">
            ...
            <p class="text-sm text-gray-500 mt-1">Saat ini belum ada event yang ditambahkan oleh pengelola.</p>
        </div>
    <?php endif; ?>
  ...
  <section class="max-w-7xl mx-auto px-6 py-14 fade-in visible">
    <div class="flex items-center justify-between mb-8">
      ...
      <a href="#" class="text-navy-mid font-semibold hover:text-gold transition-colors text-sm flex items-center gap-1">
        Lihat semua
    ...
    <?php if ($events_rekomendasi && mysqli_num_rows($events_rekomendasi) > 0): ?>
        <!-- LOOPING DATA DINAMIS DARI DATABASE -->
        <?php while($row = mysqli_fetch_assoc($events_rekomendasi)): ?>
            <article class="event-card bg-white rounded-xl shadow-md overflow-hidden cursor-pointer group relative" onclick="location.href='event-detail.php?id=<?= $row['id'] ?>'">
                <span class="badge-popular absolute top-3 right-3 z-10 bg-gold-badge text-navy-deep text-xs font-bold px-3 py-1 rounded-full">🔥 Populer</span>
                <div class="overflow-hidden h-44">
                <img src="<?= $row['gambar_cover'] ? htmlspecialchars($row['gambar_cover']) : 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=400&q=80' ?>" alt="<?= htmlspecialchars($row['judul']) ?>" class="w-full h-44 object-cover group-hover:scale-110 transition-transform duration-500" />
                </div>
                <div class="p-4 flex flex-col h-[180px]">
                <div class="flex items-center gap-2 mb-2">
                    <span class="bg-navy-mid/10 text-navy-mid text-xs px-2 py-0.5 rounded-full font-medium"><?= htmlspecialchars($row['kategori']) ?></span>
    ...
    <?php else: ?>
        ...
        <div class="col-span-1 md:col-span-2 lg:col-span-4 py-12 text-center bg-white rounded-2xl border border-gray-200 border-dashed">
            ...
            <p class="text-sm text-gray-500 mt-1">Akan muncul rekomendasi event menarik di sini.</p>
        </div>
    <?php endif; ?>
