...
<head>
  ...
  <style>
    * { font-family: 'Poppins', sans-serif; }
    ...
    .input-field:focus + .input-icon { color: #F5C400; }
    ...
    .btn-login:hover { transform: translateY(-2px); box-shadow: 0 12px 30px rgba(245,196,0,0.4); }
    ...
    .stagger > *:nth-child(5) { animation-delay: 0.43s; }
    .stagger > *:nth-child(6) { animation-delay: 0.50s; }
    .stagger > *:nth-child(7) { animation-delay: 0.57s; }
    @keyframes fadeUp {
...
<body class="min-h-screen flex flex-col">
  ...
  <main class="flex-1 flex items-center justify-center px-4 pt-28 pb-12 relative">
    ...
    <!-- Decorative ticket SVGs -->
    <svg class="ticket-deco" style="top:10%; right: 5%; width:140px;" viewBox="0 0 24 24" fill="white">
      <path d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"/>
    </svg>
    <svg class="ticket-deco" style="bottom:15%; left: 3%; width:90px; transform: rotate(-20deg);" viewBox="0 0 24 24" fill="white">
      <path d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"/>
    </svg>
      ...
      <div class="glass-card rounded-2xl p-8 md:p-10">
        ...
        <form id="login-form" method="POST" action="" class="space-y-5 stagger" novalidate>
          ...
          <div>
            ...
            <p class="error-msg text-red-400 text-xs mt-1 hidden" id="email-err"></p>
          </div>
          ...
          <div>
            ...
            <div class="input-group">
              ...
              />
              <button type="button" class="toggle-pass" id="toggle-pass" aria-label="Tampilkan password">
                <!-- Eye icon -->
            ...
            <p class="error-msg text-red-400 text-xs mt-1 hidden" id="pass-err"></p>
          </div>

          <!-- Remember + Forgot -->
          <div class="flex items-center justify-between">
            <label class="check-label flex items-center gap-2.5 cursor-pointer select-none">
              <input type="checkbox" class="sr-only" id="remember" />
              <span class="check-box">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 text-navy-deep" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                </svg>
              </span>
              <span class="text-white/60 text-sm">Ingat saya</span>
            </label>
            <a href="#" class="text-gold text-sm font-medium hover:text-gold-light transition-colors hover:underline">Lupa password?</a>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <button type="button" class="btn-social flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-medium">
  ...
  <footer class="text-center py-5 text-white/30 text-xs border-t border-white/5">
    © 2026 TicketIn. All rights reserved.
  ...
  <script>
    ...
    document.querySelectorAll('.check-label').forEach(label => {
      ...
      input.addEventListener('change', () => {
        label.querySelector('.check-box').style.background = input.checked ? '#F5C400' : '';
      });
    });
    ...
    form.addEventListener('submit', (e) => {
      ...
      // Validate email
      if (!email.value.trim()) {
        showError(email, emailErr, 'Email tidak boleh kosong');
        valid = false;
      } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)) {
        showError(email, emailErr, 'Format email tidak valid');
        valid = false;
      }

      // Validate password
      if (!pass.value) {
        showError(pass, passErr, 'Password tidak boleh kosong');
        valid = false;
      } else if (pass.value.length < 6) {
        showError(pass, passErr, 'Password minimal 6 karakter');
        valid = false;
      }
